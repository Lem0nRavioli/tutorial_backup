...
def compare_fresh_pretrained():
    model_frozen = make_model()
    model_fresh = make_model()
    train_model_pretrained_embedding(model_frozen)
    train_model_fresh_embedding(model_fresh)
    # pretrained perform a bit better with fewer samples, but outperformed quickly with more
    # 200 samples => frozen 55% acc vs 50% acc for fresh, 2000 samples, frozen 65% vs fresh 75%
