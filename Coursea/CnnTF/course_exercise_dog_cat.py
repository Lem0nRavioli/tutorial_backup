...
def split_data(SOURCE, TRAINING, TESTING, SPLIT_SIZE):
    ...
    all_dir = random.sample(os.listdir(SOURCE), len(os.listdir(SOURCE)))
    print(SOURCE)
    print(len(all_dir))
    split = int(len(all_dir) * SPLIT_SIZE)
    dir_train = all_dir[:split]
    dir_test = all_dir[split:]
    print('dir_train', len(dir_train))
    print('dir_test', len(dir_test))

    for src in dir_train:
        if os.path.getsize(os.path.join(SOURCE, src)) != 0:
            copyfile(os.path.join(SOURCE, src), os.path.join(TRAINING, src))
    for src in dir_test:
        if os.path.getsize(os.path.join(SOURCE, src)) != 0:
            copyfile(os.path.join(SOURCE, src), os.path.join(TESTING, src))
