""" generating windowed dataset and fidget with it with DNN
    lr data visualization and handpicking it
    lr call backs"""
...
from tuto_utils.util_func import show_me_lr
...
def lireg_model():
    ...
    model.compile(loss='mse', optimizer=tf.keras.optimizers.SGD(lr=1e-6, momentum=.9))

    print(f"Layer weights {l0.get_weights()}")
    return model


def simple_dnn_model(lr=1e-6):
